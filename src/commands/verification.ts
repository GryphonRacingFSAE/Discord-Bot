                ...
                .addSubcommand(sub_command =>
                    sub_command
                        ...
                        .addStringOption(option =>
                            option.setName("target").setDescription("Full name to target. If no name exists, a new will be made and you will be notified of it.").setRequired(true),
                        )
                        ...
                        await pullSpreadsheet(verification_spreadsheet)
                            .then(() =>
                                verification_spreadsheet.mutex.runExclusive(async () => {
                                    ...
                                    if (new_data) {
                                        await interaction.reply({
                                            content: `Edited \`${TARGET}\`'s data. However this is never seen before data and as such a new row has been created.`,
                                            ephemeral: true,
                                        });
                                    } else {
                                        await interaction.reply({
                                            content: `Edited \`${TARGET}\`'s data`,
                                            ephemeral: true,
                                        });
                                    }
                                }),
